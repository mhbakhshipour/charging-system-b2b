...
from charging_system_b2b.utils.model_to_dict import model_to_dict
from charging_system_b2b.middlewares import current_request
...
class LogRecord:
    ...
    def generate_log(self) -> list:
        ...
        result = self.prepare_result(old_data, new_data, old_histories, request_user)

        return result

    def __prepare_old_data(self) -> Union[dict, None]:
    ...
    def __get_new_data_from_model(self, instance, old_data: dict) -> dict:
        new_data = model_to_dict(
            instance, fields=list(old_data.keys()), get_data_with_str=True
        )

        return new_data

    def prepare_request_user(self) -> Union[User, None]:
    ...
    def __user_serializer(self, request_user: Union[User, None]) -> dict:
        if request_user:
            serialized_user = {
                "id": request_user.id,
